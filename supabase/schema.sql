...
create table if not exists public.characters (
  ...
  status text not null default 'draft' check (status in ('draft','pending','changes_requested','approved','denied')),
...
-- Duplicate-proof character ownership:
-- one user can only have one active/approved character record at a time.
-- Denied records do not block a fresh attempt.
...
-- Batman and Robin are staff-controlled single-slot mantles.
create unique index if not exists characters_unique_limited_mantle_idx
on public.characters ((data->'basic'->>'mantle'))
where status in ('draft','pending','changes_requested','approved')
  and (data->'basic'->>'mantle') in ('batman','robin');
